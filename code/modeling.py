...
class TrainConfig(PetConfig):
    ...
    def __init__(self, device: str = None, per_gpu_train_batch_size: int = 8, per_gpu_unlabeled_batch_size: int = 8,
                 ...
                 alpha: float = 0.9999, temperature: float = 1, logging_steps: int = 50):
        """
...
def train_pet(ensemble_model_config: WrapperConfig, ensemble_train_config: TrainConfig,
    ...
    # 对InputExample类的logits进行重新赋值，对应论文中的Tc数据集之前的加权
    for example, example_logits in zip(unlabeled_data, logits):
        example.logits = example_logits
    ...
    # 在SC分类器这里，将logits定位True，使用unlabelled data进行训练，标签为softmax的加权结果
    final_train_config.use_logits = True
    ...
    for pattern_id in pattern_ids:
        for iteration in range(repetitions):
            ...
            if os.path.exists(pattern_iter_output_dir):
                logger.warning(f"Path {pattern_iter_output_dir} already exists, your results will be overwrited!!")
                # continue
            ...
            if do_train:
                ...
                if save_unlabeled_logits:
                    res = evaluate(wrapper, unlabeled_data, eval_config)
                    logits = res['logits']
...
def train_single_model(model: TransformerModelWrapper, train_data: List[InputExample], config: TrainConfig,
                       ...
                       unlabeled_data: List[InputExample] = None, train_eval_data: List[InputExample] = None, return_train_set_results: bool = True):
    """
    ...
    if train_data and return_train_set_results:
        ...
        # results_dict['train_set_before_training'] = evaluate(model, train_data, eval_config)
    ...
    if not all_train_data and not config.use_logits:
        ...
        logger.info(results_dict)
    # print('line 465',results_dict)
    if train_data and return_train_set_results:
        results_dict['train_set_after_training'] = evaluate(model, train_data, eval_config)['scores']['acc']
        # results_dict['train_set_after_training'] = evaluate(model, train_data, eval_config)
    # print('*'*100)
    # print(results_dict)
...
def evaluate(model: TransformerModelWrapper, eval_data: List[InputExample], config: EvalConfig,
    ...
    for metric in metrics:
        if metric == 'acc':
            ...
            scores['acc_'] = compute_acc(predictions, results['labels'])
            scores['f1_clw'] = compute_f1_clw(predictions, results['labels'])
            scores['f1_macro'] = compute_f1_macro(predictions, results['labels'])
            scores['f1_micro'] = compute_f1_micro(predictions, results['labels'])
            scores['precision_clw'] = compute_precision_clw(predictions, results['labels'])
            scores['precision_macro'] = compute_precision_macro(predictions, results['labels'])
            scores['recall_clw'] = compute_recall_clw(predictions, results['labels'])
            scores['recall_macro'] = compute_recall_macro(predictions, results['labels'])
            scores['mcc'] = compute_mcc(predictions, results['labels'])
            scores['pearson'] = compute_pearson(predictions, results['labels'])
            scores['spearman'] = compute_spearman(predictions, results['labels'])
...
def merge_logits_lists(logits_lists: List[LogitsList], reduction: str = 'mean') -> LogitsList:
    ...
    # weights = np.array([ll.score['acc'] for ll in logits_lists])
    # 因为加了f1-score，所以这里需要指定哪个作为权重
