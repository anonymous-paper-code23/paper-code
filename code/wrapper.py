...
from .adpter import Roberta_Adapter, Roberta_Adapter_1
...
#layer.0. 'intermediate', 'key', 'query', 'value', 'output', 'output_layernorm', 'attention_layernorm', 'all', 'lm', 'ada', 'layer_norm'
# only for K-PET related models
BIAS_TERMS = ['all']
ACTIVATE_ADAPTER = True
ENCODER_TRAINABLE = True
ENCODER_UNTRAINABLE = False
if ENCODER_UNTRAINABLE and ENCODER_TRAINABLE:
...
class TransformerModelWrapper:
    ...
    def __init__(self, config: WrapperConfig):
        ...
        if self.config.model_type == 'roberta_adapter':

            self.model = Roberta_Adapter(model=self.model, model_config=model_config, trainable_components=BIAS_TERMS, encoder_trainable=True)
        if self.config.model_type == 'roberta_adapter_1':
            self.model = Roberta_Adapter_1(trainable_components=BIAS_TERMS, activate_adapter=ACTIVATE_ADAPTER,
                                           encoder_trainable=ENCODER_TRAINABLE, deactivate_trainable_para=ENCODER_UNTRAINABLE)
    ...
    def train(self, task_train_data: List[InputExample], device, per_gpu_train_batch_size: int = 8, n_gpu: int = 1,
        ...
        optimizer = AdamW(optimizer_grouped_parameters, lr=learning_rate,
                          eps=adam_epsilon)
        # optimizer = AdamW(filter(lambda p: p.requires_grad, self.model.parameters()), lr=learning_rate, eps=adam_epsilon)
        ...
        for epoch in train_iterator:
            loss_record_dict['epoch_' + str(epoch)] = []
            ...
            for _, batch in enumerate(epoch_iterator):
                ...
                tr_loss += loss.item()
                loss_record_dict['epoch_' + str(epoch)].append(loss.item())
                if (step + 1) % gradient_accumulation_steps == 0:
                    ...
                    if logging_steps > 0 and global_step % logging_steps == 0:
                        ...
                        # print('tr_loss:{0}, logging_loss:{1}, logging_steps:{2}, loss_scalar:{3}'.format(
                        #     tr_loss, logging_loss, logging_steps, loss_scalar
                        # ))
                ...
                # # 输出评估信息
                # if step % 64 == 0 and train_eval_data is not None:
                #     eval_information = self.train_eval(train_eval_data)
                #     logger.info('='*60 + 'eval results' + '='*60)
                #     logger.info(eval_information['scores'])
                #     # logger.info('loss:', list(loss))
            ...
            # frozen_parameters = {}
            # for name, p in self.model.named_parameters():
            #     frozen_parameters[name] = copy.deepcopy(p.data)
            # self.model.eval()
            # with torch.no_grad():
            #     print('test')
            ...
            #     logger.info('=' * 60 + 'eval results' + '=' * 60)
            #     logger.info(eval_information['scores'])
            #     if eval_information['scores']['f1-macro'] > best_eval['f1-macro']:
            #         best_eval = eval_information['scores']
        ...
        # logger.info('=' * 60 + 'best eval results' + '=' * 60)
        # logger.info(best_eval)

        # print('Loss per epoch: ',loss_record_dict)

        return global_step, (tr_loss / global_step if global_step > 0 else -1)

    def train_eval(self, eval_data: List[InputExample]) -> Dict:
        metrics = ['acc', 'f1-macro']
        device = torch.device("cuda")
        # logger.info('*'*100)
        # self.model.to(device)
        # logger.info('*' * 100)
        #device=args.device, n_gpu=args.n_gpu, metrics=args.metrics,
                              # per_gpu_eval_batch_size=args.sc_per_gpu_eval_batch_size
        results = self.eval(eval_data, device, per_gpu_eval_batch_size=64)
    ...
    def eval(self, eval_data: List[InputExample], device, per_gpu_eval_batch_size: int = 16, n_gpu: int = 1,
        ...
        logger.info('=' * 60 + 'begin evaluation' + '=' * 60)
        start_time = time()
        ...
        logger.info('Evaluation cost time:{:.4f}'.format(cost_time))
        return {
            'indices': all_indices,
            'logits': preds,
