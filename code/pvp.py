...
class PVP(ABC):
    ...
    def __init__(self, wrapper, pattern_id: int = 0, verbalizer_file: str = None, label_dict: dict = None, seed: int = 42):
        """
        Create a new PVP.
        ...
        if verbalizer_file:
            self.verbalize = PVP._load_verbalizer_from_jsonfile(verbalizer_file, self.pattern_id, label_dict)
    ...
    def encode(self, example: InputExample, priming: bool = False, labeled: bool = False) \
        ...
        # print(example)
        # print(parts_a,parts_b)
        # print('='*100)

        kwargs = {'add_prefix_space': True} if isinstance(tokenizer, GPT2Tokenizer) or isinstance(tokenizer, RobertaTokenizer) else {}
    ...
    def convert_mlm_logits_to_verbalizer_logits(self, mlm_labels: torch.Tensor, logits: torch.Tensor) -> tuple:
        masked_logits = logits[mlm_labels >= 0]
        cls_logits = torch.stack([self._convert_single_mlm_logits_to_verbalizer_logits(ml) for ml in masked_logits])
        return cls_logits, self.mlm_logits_to_cls_logits_tensor

    def _convert_single_mlm_logits_to_verbalizer_logits(self, logits: torch.Tensor) -> torch.Tensor:
        m2c = self.mlm_logits_to_cls_logits_tensor.to(logits.device)
        # filler_len.shape() == max_fillers
        filler_len = torch.tensor([len(self.verbalize(label)) for label in self.wrapper.config.label_list],
                                  dtype=torch.float)
    ...
    @staticmethod
    def _load_verbalizer_from_jsonfile(path: str, pattern_id: int, label_dict: dict):
        ...
        with open(path, 'r') as fh:
            # print(path)
            verbalizer_json = json.load(fh)
            for key, value in verbalizer_json.items():
                verbalizers[pattern_id][label_dict[key]] = value
...
class Semeval2016t6PVP(PVP):
    ...
    # }
    VERBALIZER_A = {
        "AGAINST": ["against", "unborn", "babies", "children", "murder", "womb", "aborted", "conception", "mother", "abort", "dies", "truly", "yr", "kills", "doctors", "demand", "worth", "considered", "voice", "innocent", "born", "destroys", "eliminated", "unless"],
        "FAVOR": ["favor", "body", "pregnant", "progress", "ability", "procedure", "uterus", "waiting", "birth", "health", "chooses", "apologies", "wills", "2015", "bill", "bodies", "needed", "reduce", "medical", "decision", "choose", "check", "decisions", "judge"],
        "NONE": ["none", "work", "destroyed", "protesters", "movement", "disgrace", "following", "evil", "draw", "protesting", "entire", "left", "married", "link", "courage", "minds", "team", "likely", "puff", "ur", "anyway", "going", "parents"]
    }
    VERBALIZER_B = {
        "AGAINST": ["opposition"],
        "FAVOR": ["support"],
    ...
    def get_parts(self, example: InputExample) -> FilledPattern:
        ...
        if self.pattern_id == 0 or self.pattern_id == 2:
            ...
            return [text_a], ['so, I stay ', self.mask, 'towards', text_b]

    def verbalize(self, label) -> List[str]:
        if self.pattern_id == 0 or self.pattern_id == 1:
            return Semeval2016t6PVP.VERBALIZER_A[label]
        return Semeval2016t6PVP.VERBALIZER_B[label]

class ArgMinPVP(PVP):
    VERBALIZER_A = {
        "Argument_against": ['wrong', 'problem', 'one', 'example', 'effect', '18', 'point', '17', 'needed', 'know', 'expressed', 'forward', 'begins', 'around'],
        "Argument_for": ['Right', 'limit', 'educational', 'last', 'including', 'order', '23', 'given', 'funding', 'start', 'course', 'days', 'stop', 'correct'],
    }
    VERBALIZER_B = {
        "Argument_against": ["against"],
        "Argument_for": ["support"],
    }
    ...
    def verbalize(self, label) -> List[str]:
        if self.pattern_id == 0 or self.pattern_id == 1:
            return ArgMinPVP.VERBALIZER_A[label]
        return ArgMinPVP.VERBALIZER_B[label]

class Iac1PVP(PVP):
    VERBALIZER_A = {
        "anti": ["Wrong",'survival', 'scientists', 'truth', 'quote', 'book', 'faith', 'murder', 'eye', 'execute', 'criminal', 'revenge', 'society', 'behavior', 'polygamy', 'vote', 'firearm', 'police', 'defense', 'ban', 'shoot', 'assault', 'accounts', 'tobacco', 'private', 'sales', 'tier', 'womb', 'conception'],
        "pro": ["Right",'fossils', 'bible', 'theories', 'organisms', 'observed', 'genesis', 'execution', 'innocence', 'inmates', 'murderers', 'sentence', 'guilty', 'straight', 'religious', 'equality', 'relationships', 'plane', 'second', 'carry', 'ownership', 'laws', 'terrorist', 'costs', 'employers', 'premiums', 'businesses', 'plans', 'deductible', 'mental', 'choice', 'moral'],
        "other": ["Maybe",'informative', 'corners','mildly', '217', 'circle', 'deleting', 'cemetery', 'pessimistic', 'deterrence', 'bombing', 'rebellious', 'hitherto', 'colourful', 'mastermind', 'stubborn', 'usable', 'closure', 'inspectors', 'senate', 'ac', 'assembly', 'inspections', 'fateful', 'fps', 'tu', 'accents', 'recruits', 'storms', 'suppliers', 'vivo', 'french', 'disband', 'showcasing', 'aluminium', 'treasury', 'ss', 'spine', 'spinal', 'jaw', 'shortage', 'scam', 'sleeper', 'alignment', '193', 'raided', 'hockey', 'certificates', 'veterinary', 'repaid', 'whereabouts', 'doi', 'right', 'mission', 'republican', 'wrong', 'pleaded', 'slavery', 'ocean', 'hanged', 'scandal', 'treason', 'endowed', 'subjective', 'rights', 'hinted', 'alienated', 'tore', 'creator']
    }
    VERBALIZER_B = {
        "anti": ["opposition"],
        "pro": ["support"],
    ...
    def verbalize(self, label) -> List[str]:
        if self.pattern_id == 0 or self.pattern_id == 1:
            return Iac1PVP.VERBALIZER_A[label]
        return Iac1PVP.VERBALIZER_B[label]
...
class FNC1PVP(PVP):
    ...
    # }
    VERBALIZER_A = {
        "unrelated": ["unrelated", "apple", "watch", "gold", "boy", "dog", "woods", "device", "father", "edition", "cook", "tour", "animals", "company", "garden", "names", "service"],
        "discuss": ["discuss", "group", "brown", "government", "militant", "girls", "video", "officer", "air", "journalist", "kidnapped", "ceasefire", "audio", "release"],
        "agree": ["agree", "dead", "penis", "parliament", "hoax", "reps", "helmet", "organ", "mobs", "bank", "parents", "lil", "fish", "blade", "marine", "horrifying", "found"],
        "disagree": ["disagree", "spider", "skin", "fireball", "crawl", "asteroid", "crater", "explosion", "mammal", "meteor", "survive", "rc", "priest", "offender", "homeless", "sciences"]
    }
    VERBALIZER_B = {
        "unrelated": ["unrelated"],
        "discuss": ["discussing"],
    ...
    def get_parts(self, example: InputExample) -> FilledPattern:
        ...
        if self.pattern_id == 0 or self.pattern_id == 2:
            ...
            return [text_a], ['so, I think it is ', self.mask, 'towards', text_b]

    def verbalize(self, label) -> List[str]:
        if self.pattern_id == 0 or self.pattern_id == 1:
            return FNC1PVP.VERBALIZER_A[label]
        return FNC1PVP.VERBALIZER_B[label]

class ARCPVP(PVP):
    VERBALIZER_A = {
        "unrelated": ["None",'information', 'immigration', 'cultural', 'thumbs', 'accounting', 'publishing'],
        "discuss": ["Maybe", 'takeover', 'cis', 'prince', 'downgrade', 'stink', 'signal'],
        "agree": ["Right",'capital', 'obese', 'writing', 'citizenship', 'polygamy', 'spelling'],
        "disagree": ["Wrong",'comments', 'libraries', 'library', 'banks', 'trump', 'gateway']
    }
    VERBALIZER_B = {
        "unrelated": ["none"],
        "discuss": ["maybe"],
    ...
    def verbalize(self, label) -> List[str]:
        if self.pattern_id == 0 or self.pattern_id == 1:
            return ARCPVP.VERBALIZER_A[label]
        return ARCPVP.VERBALIZER_B[label]

class PERSPECTRUMPVP(PVP):
    VERBALIZER_A = {
        "UNDERMINE": ["No", 'vote', 'meat', 'personal', 'government', 'cause', 'game', 'policy', 'away', 'affect',
                      'consequences', 'sixteen', 'slippery', 'slope', 'trust', 'still', 'turkey', 'inconsistent',
                      'immature', 'unstable', 'upon'],
        "SUPPORT": ["Yes", 'diet', 'crime', 'monarchy', 'bay', 'help', 'women', 'reduced', 'reduce', 'equal', 'future',
                    'yes', 'equality', 'murder', 'participation', 'athletes', 'leader', 'spent', 'aid', 'execution',
                    'magazines'],
    }
    VERBALIZER_B = {
        "UNDERMINE": ["against"],
    ...
    def get_parts(self, example: InputExample) -> FilledPattern:
        ...
        if self.pattern_id == 0 or self.pattern_id == 2:
            ...
            return [text_a, '. '], ['Based on the previous information, ', text_b, '?', self.mask, '.']
        elif self.pattern_id == 6 or self.pattern_id == 7:
            return [text_a, '?'], [self.mask, ', ', text_b]

    def verbalize(self, label) -> List[str]:
        if self.pattern_id == 0 or self.pattern_id == 1 or self.pattern_id == 4 or self.pattern_id == 6:
            return PERSPECTRUMPVP.VERBALIZER_A[label]
        return PERSPECTRUMPVP.VERBALIZER_B[label]
...
class IbmcsPVP(PVP):
    ...
    # }
    VERBALIZER_A = {
        "CON": ["con", "reservoirs", "dam", "party", "first", "large", "disproportionate", "condoms", "amendment", "reservoir", "failures", "dams", "primary", "social", "violation", "vulnerable", "construction"],
        "PRO": ["pro", "advertising", "energy", "rocket", "tobacco", "renewable", "less", "economic", "fuel", "software", "contraception", "forms", "proprietary", "fossil", "resources", "cost", "technologies", "quality"],
    }
    VERBALIZER_B = {
        "CON": ["against"],
        "PRO": ["support"],
    ...
    def get_parts(self, example: InputExample) -> FilledPattern:
        ...
        if self.pattern_id == 0 or self.pattern_id == 2:
            ...
            return [text_a], ['so, I ', self.mask, text_b]

    def verbalize(self, label) -> List[str]:
        if self.pattern_id == 0 or self.pattern_id == 1:
            return IbmcsPVP.VERBALIZER_A[label]
        return IbmcsPVP.VERBALIZER_B[label]
...
class SnopesPVP(PVP):
    ...
    # }
    VERBALIZER_A = {
        "refute": ["refute", "rumor", "false", "hoax", "never", "true", "story", "evidence", "rumors", "circulating", "email", "information", "nothing", "said", "scientific", "myth", "truth"],
        "agree": ["agree", "trump", "man", "president", "morning", "today", "minutes", "send", "reportedly", "year", "two", "pictures", "announced", "discovered", "old", "presidential", "new"],
    }
    VERBALIZER_B = {
        "refute": ["against"],
        "agree": ["support"],
    ...
    def get_parts(self, example: InputExample) -> FilledPattern:
        ...
        if self.pattern_id == 0 or self.pattern_id == 2:
            ...
            return [text_a], ['I ', self.mask, ',', text_b]

    def verbalize(self, label) -> List[str]:
        if self.pattern_id == 0 or self.pattern_id == 1:
            return SnopesPVP.VERBALIZER_A[label]
        return SnopesPVP.VERBALIZER_B[label]

class Semeval2019t7PVP(PVP):
    VERBALIZER_A = {
        "support": ["Yes"],
        "deny": ["No"],
        "query": ["doubt"],
    ...
    def verbalize(self, label) -> List[str]:
        if self.pattern_id == 0 or self.pattern_id == 1:
            return Semeval2019t7PVP.VERBALIZER_A[label]
        return Semeval2019t7PVP.VERBALIZER_B[label]


class PstancePVP(PVP):
    # VERBALIZER_A = {
    #     "AGAINST": ["Wrong"],
    #     "FAVOR": ["Right"],
    #     "NONE": ["Maybe"]
    ...
    def verbalize(self, label) -> List[str]:
        if self.pattern_id == 0 or self.pattern_id == 1:
            return PstancePVP.VERBALIZER_A[label]
        return PstancePVP.VERBALIZER_B[label]
...
PVPS = {
    ...
    'ax-b': RtePVP,
    'ax-g': RtePVP,
    '2016t6': Semeval2016t6PVP,
    'argmin': ArgMinPVP,
    'iac1': Iac1PVP,
    'fnc1': FNC1PVP,
    'perspectrum': PERSPECTRUMPVP,
    '2019t7': Semeval2019t7PVP,
    'arc': ARCPVP,
    'ibmcs': IbmcsPVP,
    'snopes': SnopesPVP,
    'pstance': PstancePVP,
